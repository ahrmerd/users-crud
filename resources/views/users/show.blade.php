...
@section('content')
    <div class="h-full flex justify-center items-center ">
        <div
            ...
            <a class="scale-50 absolute top-0 right-0 " href="{{ route('users.edit', $user->id) }}">
            ...
            <div class="space-y-4 text-center divide-y divide-gray-700">
                <div class="my-2 space-y-1">
                    <h2 class="text-xl font-semibold sm:text-2xl">{{ $user->first_name }}
                        {{ $user->last_name }}</h2>
                    <p class="text-xs sm:text-base">{{ $user->email }}</p>

                    <p class="text-xs
                        sm:text-base">{{ $user->username }}</p>

                </div>
                <div class="flex
                        justify-center pt-2 space-x-4 align-center">
                    <span class="px-5 text-xs sm:text-base dark:text-gray-400">{{ $user->is_admin ? 'Admin' : 'User' }}
                        @if ($user->is_admin)
